...
export function PostThread({ post, onClose }: { post: Post; onClose: () => void }) {
  ...
  return <section className="thread-panel" aria-label="Post conversation">
    ...
    <article className="thread-original"><div className="thread-author"><span className="avatar" style={{ background: post.color }}>{post.name.split(' ').map(part => part[0]).join('')}</span><div><strong>{post.name}</strong><span>@{post.handle}</span></div></div>{post.text&&<p>{post.text}</p>}{post.image&&<img className="thread-image" src={post.image} alt={post.imageAlt||`Photo shared by ${post.name}`}/>}<small>Today at {post.time === 'now' ? 'just now' : post.time} · Helsinki</small><footer><span><MessageCircle size={16} /> {post.replies + replies.length}</span><span><Heart size={16} /> {post.likes}</span></footer></article>
