...
function App(){
  ...
  const [query,setQuery]=useState('');const [toast,setToast]=useState('');const [following,setFollowing]=useState<string[]>(['emilialaine','oskarin'])
  ...
  const visible=useMemo(()=>posts.filter(post=>`${post.name} ${post.handle} ${post.text}`.toLowerCase().includes(query.toLowerCase())&&(feedMode==='all'||following.includes(post.handle))).sort((first,second)=>Number(Boolean(second.pinned))-Number(Boolean(first.pinned))),[posts,query,feedMode,following])
  ...
  const publish=(text:string,image?:string,imageAlt?:string)=>{setPosts(items=>[{id:Date.now(),name:'Umut Efe',handle:'umutefe',time:'now',color:'#b9e678',text,likes:0,replies:0,image,imageAlt},...items]);notify(image?'Your photo is now in the feed':'Your post is live')}
  const follow=(handle:string)=>setFollowing(items=>items.includes(handle)?items.filter(i=>i!==handle):[...items,handle])
  const pinPost=(post:Post)=>{setPosts(items=>items.map(item=>item.handle==='umutefe'?{...item,pinned:item.id===post.id?!post.pinned:false}:item));setOpenPostMenu(null);notify(post.pinned?'Post unpinned':'Post pinned to your profile')}
  const savePost=(changes:Partial<Post>)=>{if(!editingPost)return;update(editingPost.id,changes);setEditingPost(null);notify('Your changes are saved')}
  const deletePost=()=>{if(!deletingPost)return;setPosts(items=>items.filter(item=>item.id!==deletingPost.id));setDeletingPost(null);notify('Post deleted')}
  const toggleBookmark=(post:Post)=>{update(post.id,{bookmarked:!post.bookmarked});notify(post.bookmarked?'Removed from bookmarks':'Saved to bookmarks')}
  ...
  return <div className="app">
    ...
    <main id="feed"><header className="feed-header"><div><h1>Home</h1><span>For you</span></div><button aria-label="Feed settings"><Settings size={18}/></button></header>
      ...
      {visible.map(post=><article className={`post ${post.pinned?'pinned-post':''}`} key={post.id}><Avatar initials={post.name.split(' ').map(n=>n[0]).join('')} color={post.color}/><div className="post-body">{post.pinned&&<span className="pinned-label">Pinned to your profile</span>}<header onClick={()=>setSelectedPost(post)}><strong>{post.name}</strong><span>@{post.handle} · {post.time}</span>{post.handle==='umutefe'&&<button aria-label="Post options" aria-expanded={openPostMenu===post.id} onClick={event=>{event.stopPropagation();setOpenPostMenu(current=>current===post.id?null:post.id)}}><MoreHorizontal size={18}/></button>}{openPostMenu===post.id&&<PostMenu post={post} onClose={()=>setOpenPostMenu(null)} onEdit={()=>{setEditingPost(post);setOpenPostMenu(null)}} onPin={()=>pinPost(post)} onDelete={()=>{setDeletingPost(post);setOpenPostMenu(null)}}/>}</header>{post.text&&<p onClick={()=>setSelectedPost(post)}>{post.text}</p>}{post.image&&<button className="uploaded-photo" onClick={()=>setSelectedPost(post)}><img src={post.image} alt={post.imageAlt||`Photo shared by ${post.name}`}/></button>}{post.photo&&<div className="post-photo" onClick={()=>setSelectedPost(post)}><span>Late afternoon<br/><strong>Helsinki, Finland</strong></span></div>}<footer><button onClick={()=>setSelectedPost(post)}><MessageCircle size={16}/><span>{post.replies}</span></button><button onClick={()=>setSharingPost(post)}><Repeat2 size={17}/><span>Share</span></button><button className={post.liked?'liked':''} onClick={()=>update(post.id,{liked:!post.liked,likes:post.likes+(post.liked?-1:1)})}><Heart size={17} fill={post.liked?'currentColor':'none'}/><span>{post.likes}</span></button><button className={post.bookmarked?'bookmarked':''} onClick={()=>toggleBookmark(post)} aria-label={post.bookmarked?'Remove bookmark':'Bookmark post'}><Bookmark size={16} fill={post.bookmarked?'currentColor':'none'}/></button></footer></div></article>)}
    </main>
    ...
    {sharingPost&&<ShareDialog post={sharingPost} conversations={messages.conversations} onSend={messages.sendMessage} onNotice={notify} onClose={()=>setSharingPost(null)}/>} {editingPost&&<EditPostDialog post={editingPost} onSave={savePost} onClose={()=>setEditingPost(null)}/>} {deletingPost&&<DeletePostDialog onDelete={deletePost} onClose={()=>setDeletingPost(null)}/>} {view!=='Home'&&<SocialPanel view={view} onClose={()=>setView('Home')} notifications={notifications} onRead={id=>setNotifications(items=>items.map(item=>item.id===id?{...item,read:true}:item))} onReadAll={()=>setNotifications(items=>items.map(item=>({...item,read:true})))} onDismiss={id=>setNotifications(items=>items.filter(item=>item.id!==id))} conversations={messages.conversations} activeConversationId={messages.activeId} onOpenConversation={messages.openConversation} onSendMessage={messages.sendMessage} posts={posts} following={following} onFollow={follow} onOpenPost={post=>{setView('Home');setSelectedPost(post)}} onRemoveBookmark={toggleBookmark}/>} {selectedPost&&<PostThread post={selectedPost} onClose={()=>setSelectedPost(null)}/>} {toast&&<div className="toast" role="status">{toast}</div>}
  </div>
}
