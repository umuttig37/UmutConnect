...
function App(){
  ...
  return <div className="app">
    ...
    {view!=='Home'&&<SocialPanel view={view} onClose={()=>setView('Home')} notifications={notifications} onRead={id=>setNotifications(items=>items.map(item=>item.id===id?{...item,read:true}:item))} onReadAll={()=>setNotifications(items=>items.map(item=>({...item,read:true})))} onDismiss={id=>setNotifications(items=>items.filter(item=>item.id!==id))}/>} {selectedPost&&<PostThread post={selectedPost} onClose={()=>setSelectedPost(null)}/>} {toast&&<div className="toast" role="status">{toast}</div>}
  </div>
}
